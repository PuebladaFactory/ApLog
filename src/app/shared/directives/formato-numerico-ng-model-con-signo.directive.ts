...
@Directive({
  selector: '[appFormatoNumericoNgModelConSigno]',
  standalone: false
})
export class FormatoNumericoNgModelConSignoDirective implements AfterViewInit, OnInit  {
 ...
 private puntoIngresadoPorUsuario = false; // Indicador para el punto del usuario
  ...
  onKeyDown(event: KeyboardEvent): void {
    ...
    if (event.key === '.') {
      ...
      // Permitir + o - solo si el cursor está al inicio y no existe ya un signo
      const valorActual = input.value;

      if (input.selectionStart !== 0 || valorActual.startsWith('+') || valorActual.startsWith('-')) {
        this.mostrarError(input, 'El signo debe estar al inicio y solo una vez');
        event.preventDefault();
      }

    } else if (!/[\d,]/.test(event.key)) {
      // Bloquear cualquier otro carácter no permitido
      this.mostrarError(input, 'Solo se permiten números, punto, coma y signo');
      event.preventDefault();
    }
  }
  ...
  onModelChange(value: any): void {
    ...
    /* if (value) {
      ...
      valor = tieneSigno ? valor.substring(1) : valor; // quitar signo para formatear
      ...
      //this.renderer.setProperty(input, 'value', valor);
      this.renderer.setProperty(this.el.nativeElement, 'value', signo + valor);
