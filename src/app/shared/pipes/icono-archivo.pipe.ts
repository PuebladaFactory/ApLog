...
const SVG_ICONS: { [key: string]: string } = {
...
@Pipe({
  name: 'iconoArchivo',
  standalone: false, // mismo criterio que el resto de los pipes del proyecto (SafeUrlPipe, etc.)
})
export class IconoArchivoPipe implements PipeTransform {
  constructor(private sanitizer: DomSanitizer) {}

  transform(fileName: string): SafeHtml {
    const extension = this.getFileExtension(fileName);
    const svg = SVG_ICONS[extension] || SVG_ICONS['default'];
