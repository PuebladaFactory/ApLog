...
/*     {
    ...
    },
    {
      ...
      align: "center",
      value: (inf) =>
        inf.estadoFinanciero === "cobrado" || inf.estadoFinanciero === "parcial"
  ...
  aplicarFiltros() {
    ...
    this.informesFiltrados = this.informesLiq.filter((inf) => {
      ...
      const estado = inf.estado?.toLowerCase().includes(textoBusqueda);

      const estadoFinanciero = inf.estadoFinanciero
        ?.toLowerCase()
        .includes(textoBusqueda);

      const periodo = inf.periodo?.toLowerCase().includes(textoBusqueda);

      const mes = inf.mes?.toLowerCase().includes(textoBusqueda);

      const anio = inf.anio?.toString().toLowerCase().includes(textoBusqueda);
  ...
  descargarFactura(inf: InformeLiq) {
    if (!inf.facturaUrl) {
      Swal.fire(
        "Sin factura",
        "El informe no tiene factura electrónica.",
        "info",
      );
      return;
    }
  ...
  migrarInformesAFinanzas(informes: ConId<InformeLiq>[]): ConId<InformeLiq>[] {
  ...
  actualizarObjeto() {
    this.cargando = true;
    this.dbService
      .actualizarOperacionesBatch(this.informesEditables, "resumenLiq")
      .then((result) => {
        this.cargando = false;
        if (result.exito) {
