...
export class ResumenOpLiquidadasComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.facLiquidadas.forEach((f: InformeOp) => {
      this.totalContraParte += f.contraParteMonto;
    });
  ...
  getProveedor() {
    ...
    proveedorArray = this.proveedores.filter((p: Proveedor) => {
      return p.idProveedor === this.facLiquidadas[0].idProveedor;
    });
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          let valores = {
            ...
            totalkmMonto: 0,
            total: this.total,
            descuentoTotal: this.totalDescuento,
            totalContraParte: this.totalContraParte,
            totalAdExtra: 0,
          };
          this.facLiquidadas.forEach((f: InformeOp) => {
  ...
  async generarInformeLiquidacion(
    ...
    const numero =
      accion === "factura"
        ? await this.numeradorService.generarNumeroInterno(
            this.fromParent.origen,
          )
        : "";
    let idInforme =
      this.fromParent.origen === "cliente"
        ...
        : this.fromParent.origen === "chofer"
          ? this.choferSel.idChofer
          : this.proveedorSel.idProveedor;
    let razonSocial =
      this.fromParent.origen === "cliente"
