...
export class FacturacionChoferComponent implements OnInit {
  ...
  informeLiqChofer!: ConId<InformeLiq>;  
  ...
  isLoading: boolean = false;
    ...
    procesarDatosParaTabla() {
      ...
      //////console.log()(this.informesLiqChofer);
      
      if(this.informesLiqChofer !== null){
        ...
        this.informesLiqChofer.forEach((informeLiq: InformeLiq) => {
          if (!choferesMap.has(informeLiq.entidad.id)) {
            choferesMap.set(informeLiq.entidad.id, {
              idChofer: informeLiq.entidad.id,            
              chofer: informeLiq.entidad.razonSocial,            
              sumaAPagar: 0,
              sumaACobrar: 0,
              faltaPagar: 0,
              total: 0,
          ...
          const chofer = choferesMap.get(informeLiq.entidad.id);
          //cliente.sumaACobrar++;
          if (informeLiq.cobrado) {
            chofer.sumaAPagar += Number(informeLiq.valores.total);
          } else {
            chofer.sumaAPagar += Number(informeLiq.valores.total);
            chofer.faltaPagar += Number(informeLiq.valores.total);
          }
          chofer.total += Number(informeLiq.valores.total);
          chofer.sumaACobrar += Number(informeLiq.valores.totalContraParte);  
          chofer.cant += Number(informeLiq.operaciones.length);    
    ...
    borrarLiquidaciones(){
        this.isLoading = true;
        console.log("this.informesLiqChofer", this.informesLiqChofer);
        this.dbFirebase.eliminarMultiple(this.informesLiqChofer, "informeLiqChofer").then((result)=>{
