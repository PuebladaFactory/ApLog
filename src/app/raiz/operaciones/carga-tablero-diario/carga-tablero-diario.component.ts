...
export class CargaTableroDiarioComponent implements OnInit, OnDestroy {
    ...
    for (const entrada of entradas) {
      ...
      for (const chofer of entrada.choferes as ChoferAsignado[]) {
        ...
        const op = {
          ...
          tarifaOverride: chofer.tEventual || cliente.tarifaTipo?.eventual ? 'eventual' : null
  ...
  calcularTarifaBase(t: Operacion['tarifaTipo']): OperacionRuntime['tarifaBase'] {
  ...
  validarOperacion(op: OperacionRuntime): string[] {
    ...
    if (activa === 'eventual') {
      if (!op.tarifaEventual.chofer.concepto) errores.push('Concepto chofer eventual faltante');
      if (!op.tarifaEventual.cliente.concepto) errores.push('Concepto cliente eventual faltante');
    }

    if (activa === 'personalizada') {
      if (op.tarifaPersonalizada.seccion <= 0) errores.push('Sección personalizada faltante');
      if (op.tarifaPersonalizada.categoria <= 0) errores.push('Categoría personalizada faltante');
    }
  ...
  guardar(): void {
    ...
    if (errores.length) {
      Swal.fire({ icon: 'error', title: 'Validación', html: errores.join('<br>') });
      return;
    }
    ...
    this.activeModal.close({ operaciones: operacionesFinales, asignaciones });
  }
