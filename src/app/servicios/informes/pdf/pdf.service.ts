...
export class PdfService {
  ...
  async exportToPdfInforme(
    ...
    const idFactura = modo === 'factura' ? `ID: ${informeLiq.numeroInterno}`: `ID: ${informeLiq.idInfLiq}`;
    const nombreDoc = modo === 'factura' ? `Detalle_${informeLiq.entidad.razonSocial}_${informeLiq.fecha}.pdf` : `Proforma_${informeLiq.entidad.razonSocial}_${informeLiq.fecha}.pdf`
  ...
  const filas = informesOp.map((fila) =>
    encabezado.map((columna) => {
      const valor = this.obtenerDatos(informeLiq, fila, clientes, columna, choferes);
  ...
  doc.text(`${this.formatearValor(informeLiq.valores.total)}`, margin + tableWidth - (informeLiq.columnas.length <= 9 ? 3 : 2), finalY + footerHeight / 2, {
    align: 'right',
    baseline: 'middle',
  });
