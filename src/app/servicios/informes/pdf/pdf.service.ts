...
async exportToPdfCliente(
  ...
  autoTable(doc, {
    ...
    margin: { top: 45 },
    foot: this.generarFooter(factura),
    footStyles: {
      fillColor: '#6495ED',
      textColor: 'black',
      //fontStyle: 'bold',
...
async exportToPdfChofer(factura: FacturaChofer, facturasOp: FacturaOp[], clientes: Cliente[], choferes:Chofer[]): Promise<void> {
  ...
  doc.setFontSize(12); // Tamaño de fuente del título
  doc.setFont('helvetica', 'bold');
  doc.text(`Liquidación de Servicios ${factura.apellido} ${factura.nombre}`, 10, 30); // Posición (60, 15)
  ...
  // ID de la factura
  doc.setFontSize(8); // Tamaño de fuente del subtítulo
  doc.text(`ID: ${factura.idFacturaChofer}`, 10, 40); // Posición (60, 35)
  ...
  doc.save(`Factura ${factura.apellido}-${factura.nombre}-${factura.fecha}.pdf`);
}
...
async exportToPdfProveedor(factura: FacturaProveedor, facturasOp: FacturaOp[], clientes: Cliente [], choferes: Chofer[]): Promise<void> {
  ...
  // ID de la factura
  doc.setFontSize(8); // Tamaño de fuente del subtítulo
  doc.text(`ID: ${factura.idFacturaProveedor}`, 10, 40); // Posición (60, 35)
