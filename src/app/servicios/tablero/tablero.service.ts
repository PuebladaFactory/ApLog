...
private getCategoriaDesdeOperacion(op: Operacion): Categoria {
  ...
  /** 📥 Descargar informe Excel del tablero */
/*   async descargarInforme(fecha: string): Promise<void> {
    const tablero = await this.getTableroPorFecha(fecha);
    if (!tablero || !tablero.asignado) {
      Swal.fire({ icon: 'info', title: 'Sin datos', text: 'El tablero aún no fue asignado.' });
      return;
    }

    const operaciones = await this.dbFirestore.getOperacionesPorFecha(fecha);
    if (!operaciones.length) {
      Swal.fire({ icon: 'info', title: 'Sin operaciones', text: 'No hay operaciones para esta fecha.' });
      return;
    }

    await this.excelService.generarInformeAsignaciones(fecha, operaciones);
  } */
}
