...
import { MovimientoFinanciero, ImputacionMovimiento } from "src/app/interfaces/movimiento-financiero";
...
export class MovimientoFinancieroService {
  ...
  async registrarMovimientoFinanciero(
    ...
    await runTransaction(this.firestore, async (tx) => {
      ...
      const { prefijo, numero } =
        await this.numeradorService
          .leerProximoNumeroMovimiento(tx, form.tipo);

      const numeroComprobante =
        `${prefijo}-${numero.toString().padStart(6,'0')}`;
      ...
      const resultadoMotor =
        SaldoEngine.calcularImpactoMovimiento({
      ...
      const imputaciones: ImputacionMovimiento[] =
        informesLeidos.map(i => ({
          informeLiqId: i.id,
          numeroInterno: i.data.numeroInterno,
          fechaInforme: typeof i.data.fecha === 'string'
          ? i.data.fecha
  ...
  await runTransaction(this.firestore, async (tx) => {
    ...
    for (const inf of informesLeidos) {

      const valores = inf.data.valoresFinancieros!;

      const nuevoSaldo = valores.saldo + inf.monto;

      let nuevoEstado: 'pendiente' | 'parcial' | 'pagado';

      if (nuevoSaldo === valores.total) {
